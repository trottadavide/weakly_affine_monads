...
\begin{mylemma}\label{threeindep}
...
\todo[inline]{This feels vaguely like Yoneda, but in monoidal sauce. Can't make it precise for now, though.}
...
\begin{mylemma}
 Let $\mD$ be a cartesian monoidal category, and let $T$ be a commutative monad on $\mD$ where $T1$ with its canonical monoid structure is a group (or equivalently, where the Kleisli category is weakly Markov).
 Then \eqref{c_assoc_pullback} is a pullback. 
\end{mylemma}

\todo[inline]{proof still incomplete}

\begin{proof}
By the universal property of products, a cone over the cospan in \eqref{c_assoc_pullback} consists of maps $g_1^\sharp:A\to TX$, $g_{23}^\sharp:A\to T(Y\times Z)$, $g_{12}^\sharp:A\to T(X\times Y)$ and $g_3^\sharp:A\to TZ$ such that the following diagram commutes. 
 \[
  ...
  \end{tikzcd}
 \]
 By \Cref{indepkleisli}, this amounts to a Kleisli map $f^\sharp:A\to T(X\times Y\times Z)$ exhibiting conditional independence of $X$ and $Y\otimes Z$ given $A$, as well as of $X\otimes Y$ and $Z$ given $A$. By \Cref{threeindep}, we then have that $f$ exhibits conditional independence of all $X$, $Y$ and $Z$ given $A$, and so, again by \Cref{indepkleisli}, $f^\sharp$ factors through the product $TX\times TY\times TZ$. 
...
\[
 ...
 Therefore there is a unique map $A\to TX\times TY\times TZ$ making the whole diagram commute, which means that \eqref{c_assoc_pullback} is a pullback.
\end{proof}
...
\begin{mytheorem}
 ...
 \begin{enumerate}
  \item $T1$ with its canonical internal monoid structure is an internal group.
  \item \eqref{c_assoc_pullback} is a pullback
  \item The Kleisli category of $T$ is weakly Markov.
 \end{enumerate}
\end{mytheorem}
...
\end{proof}
\todo{ovviamente abbiamo anche un'azione destra.... devo finire di controllare se $\gamma$ viene l'identità o in generale no}
\begin{myproposition}
	\label{lem:group_action}
	If $T$ is weakly affine, then for every object $X$ there exists a unique morphism $\gamma:T(X)\to T(X)$ such that the composition $c_{1,X}\circ (\id_{T1}\times \gamma): T(1)\times T(X)\to T(X)$ is a (left) group action. 
