...
\begin{myproposition}
	...
	If $T$ is weakly affine, then for every object $X$ there exists a unique morphism $\gamma:T(X)\to T(X)$ such that the composition $c_{1,X}\circ (\id_{T1}\times \gamma): T(1)\times T(X)\to T(1)$ is a (left) group action. 
...
Let $\freccia{T(X)}{\gamma}{T(X)}$ be the unique morphism such that the diagram
	\[
		...
		\end{tikzcd}
	\]
	commutes. Now we show that the morphism $c_{1,X}\circ (\id_{T1}\times \gamma): T(1)\times T(X)\to T(1)$ provides an action of the group $T(1)$ on $T(X)$. First, notice that identity axiom, i.e. $(c_{1,X} (\id_{T1}\times \gamma))(\eta_1!, \id)=\id$, is satisfied by definition of $\gamma$, because $c_{1,X}(\eta_1!,\gamma)=\id$. 
