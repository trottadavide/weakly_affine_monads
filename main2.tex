...
Weakly Markov categories can be interpreted intuitively as gs-monoidal categories where each morphism is discardable up to an invertible normalisation (see \Cref{weaklydiscardable} for the precise mathematical statement). Trivially, every Markov category is weakly Markov.
...
Using these concepts, we give an equivalent condition for weak affinity of a monad, a pullback condition on the associativity diagram (\Cref{mainthm}), widely generalising the elementary statement that a monoid is a group if and only if its associativity diagram is a pullback (\Cref{assoc_group}). 
