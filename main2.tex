...
referring to \cite{Selinger2011} for an overview of various notions of monoidal categories and their associated diagrammatic calculus.
...
\begin{myproposition}\label{eqcondind}
 Let $f:A\to X_1\otimes\dots\otimes X_n$ be a morphism in a weakly Markov GS-category $\mC$. Then $f$ exhibits conditional independence of the $X_i$ given $A$ if and only if it is equivalent to the product of all its marginals.
...
The concept of conditional independence for general weakly Markov categories allow us to give an equivalent characterization of affine monads. 
...
\begin{mylemma}[localized independence property]\label{local}
 Let $\mC$ be a weakly Markov GS-category. Whenever a morphism  $f:A\to X\otimes Y\otimes Z$ exhibits conditional independence of $X\otimes Y$ (jointly) and $Z$ given $A$, as well as conditional independence of $X$ and $Y\otimes Z$ given $A$, then it exhibits conditional independence of $X$, $Y$ and $Z$ given $A$. 
\end{mylemma}
...
\begin{myexample}
 In the Kleisli category of the measure monad $\mathrm{Kl}(M)$ (which is not weakly affine) consider the following diagram.
 \[
		\hspace{-9pt}	% T: for centering the equation number
		\begin{tikzcd}[column sep=3.3pc]
...
\end{mycorollary}
\begin{proof}
By weak affineness, $T(1)$ is a group. If $\eta_{T1}=T(\eta_{1})\iota$, then we can apply the multiplication of the monad to both sides, obtaining $\iota=\id_{T1}$.
\end{proof}
...
Note that for a generic commutative monad, the diagram above may even fail to commute (take for example the measure monad $M$, and start with $(x,0)$ in the bottom left corner). One can however consider the following diagram, which reduces to the one above (up to isomorphism) in the affine case,
\[
 \begin{tikzcd}
  ...
  \end{tikzcd}
\]
and which always commutes by naturality of the strength. One can then call the monad $T$ \emph{positive} if this second diagram is a pullback (and possibly define \emph{positive GS categories} analogously to positive Markov categories). 
