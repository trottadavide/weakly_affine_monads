...
\begin{proposition}\label{affinemarkov}
 In the hypotheses of \Cref{monoidalgs}, we have that $\mathrm{Kl}_T$ is Markov if and only if $T$ is affine.
\end{proposition}

In this work, we introduce an intermediate level between gs-monoidal and Markov called \emph{weakly Markov}, and its corresponding notion for monads, which we call \emph{weakly affine}. 
...
\begin{mydefinition}
 A gs-monoidal category $\mC$ is called \emph{weakly Markov} if for every object $X$, the monoid $\mC(X,I)$ is a group. 
\end{mydefinition}
...
\begin{proof}
 ...
 \[
  \begin{tikzcd}
  X \ar[bend right=5pc]{dd}[swap]{!} \ar{d}[swap]{a^\sharp} \ar{r}{\mathrm{copy}} & X\times X \ar{d}[swap]{a^\sharp\times a^\sharp} \ar{dr}{a^\sharp\times(\iota\circ a^\sharp)} \\
  T1 \ar{d}[swap]{!} \ar{r}[swap]{\mathrm{copy}} & T1\times T1 \ar{r}[swap]{\id\times\iota} & T1\times T1 \ar{r}{c} & T(1\times 1) \ar{d}{\cong} \\
  1 \ar{rrr}{\eta} &&& T1
  \end{tikzcd}
 \]
...
\begin{enumerate}
	...
	\item $T(1)$ is an idempotent group, namely $\iota=\id_{T1}$.
\end{enumerate}
\todo[inline]{T: Idempotency usually means that $xx = x$ for every group/monoid element $x$, right? The correct term here would be exponent 2, see \href{https://math.stackexchange.com/questions/1228513/what-is-the-exponent-of-a-group}{math.stackexchange.com/questions/1228513/what-is-the-exponent-of-a-group}}
\end{myproposition}
