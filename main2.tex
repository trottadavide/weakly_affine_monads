...
\begin{abstract}
   ...
   and as such provide a general framework for effectful computation. 
   ...
   which is intermediate between general gs-monoidal categories and Markov categories. 
...
The idea of \emph{gs-monoidal categories}, monoidal categories equipped with copy and discard morphisms making every object a comonoid,
...
It is noteworthy that a notion of conditional independence can also be given for generic gs-monoidal categories. As we show, for weakly Markov categories it has convenient properties which can be considered ``up-to-normalisation'' versions of their corresponding Markov-categorical counterpart.
%
...
%
As such, we believe that weak affine monads that are relevant to the study of categorical probability, 
