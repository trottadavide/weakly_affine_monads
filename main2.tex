...
Finally, in the concluding \Cref{secfurther}, we pose further questions, such as when we can iterate the construction of weakly Markov categories by means of weakly affine monads, and the relationship with strongly affine monads in the sense of Jacobs~\cite{Jacobs16}.
...
\item generalizing other Markov-categorical notions, such as positivity, to weakly Markov or even general gs-monoidal categories.
\end{itemize} 
...
One fact to notice to avoid possible misconceptions, though, is that while a weakly affine commutative monad maps the group object $1$ to a group object, preserving its monoid structure, it does not quite preserve the inverses in general. 
To elucidate this further, let's give the following definition.

\begin{mydefinition}
 Let $\mD,\mE$ be a monoidal categories and let $F:\mD\to\mE$ be a lax monoidal functor, so that it maps monoid objects to monoid objects.
...
For example, if $F$ is \emph{strong} monoidal, it strongly preserves every existing group object structure, since the following diagram commutes,
\[
 \begin{tikzcd}[row sep=small]
  & & 1 \ar{ddrr}{e_{FG}} \ar{d}{\cong}\\
  & & F1 \ar{drr}[swap]{Fe_G} \\
  FG \ar{uurr}{!} \ar{urr}[swap]{F!} \ar{dr}{F(\mathrm{copy})} \ar{ddr}[swap]{\mathrm{copy}} & && & FG \\
...
\begin{myproposition}
 Let $T$ be a weakly affine commutative monad on a cartesian monoidal category $\mD$. The following conditions are equivalent.
 \begin{enumerate}
