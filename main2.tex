...
As a particular concrete example of relevance to probability and measure theory, we consider the monad of finite nonzero measures on $\Set$ (\Cref{ex:nonzero_measures}), and we will use it as a running example in the rest of the work.
...
These concepts allow us to provide an equivalent condition for weak affinity of a monad, namely a pullback condition on the associativity diagram (\Cref{mainthm}), widely generalising the elementary statement that a monoid is a group if and only if its associativity diagram is a pullback (\Cref{assoc_group}). 
